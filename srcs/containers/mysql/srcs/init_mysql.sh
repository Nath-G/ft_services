...
echo "CREATE USER 'mysql_admin'@'%' IDENTIFIED BY 'mysql_admin';" | mysql -u root --skip-password
echo "GRANT ALL PRIVILEGES ON wordpress.* TO 'wp_admin'@'%' WITH GRANT OPTION;" | mysql -u root --skip-password
echo "GRANT ALL PRIVILEGES ON *.* TO 'mysql_admin'@'%' WITH GRANT OPTION;" | mysql -u root --skip-password
